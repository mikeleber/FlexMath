...
public class MathEval
        extends Object {
    private static final Constant NULL_VALUE = new Constant() {
        @Override
...
// *************************************************************************************************
    ...
    private final SortedMap<String, Object> variables;                                                  // external variables
...
// *************************************************************************************************
    ...
    public Object getConstant(String nam) {
        ...
        return (val == null ? NULL_VALUE : val);
    }
     ...
     */
    public Iterable<Map.Entry<String, Object>> getVariables() {
...
// *************************************************************************************************
        ...
        for (ofs = beg; (ofs = skipWhitespace(expression, ofs, end)) <= end; ofs++) {
            ...
            EvaluateOperand:
            {
                ...
                if (beg == ofs && (cur.unary == LEFT_SIDE || nxt.unary == RIGHT_SIDE)) {
                    ...
                    int nestedEnd = StringUtils.getNestedEndPos(expression, offset + 1, end, '[', ']');                                        // skip past ')' and any following whitespace
                    rgt = createArray(beg + 1, nestedEnd - 1);
                    ofs = nestedEnd;
                    offset = ofs;
                    nxt = (ofs <= end ? getOperator(expression.charAt(ofs)) : OPERAND);

                    // modify next operator
                } else if (alp) {
    ...
    private Object doOperation(int beg, Object lft, Operator opr, Object rgt) {
        if (opr.unary != RIGHT_SIDE && (lft instanceof Double && ((Double) lft).isNaN())) {
            throw exception(beg, "Mathematical NaN detected in right-operand");
        }
        if (opr.unary != LEFT_SIDE && (rgt instanceof Double && ((Double) rgt).isNaN())) {
    ...
    private List createArray(int beg, int end) {
        String arrayString = expression.substring(beg, end);
...
// *************************************************************************************************
    ...
    static public interface Constant {
        public Object getValue();
...
// *************************************************************************************************
    ...
    static class DefaultImpl
        ...
        @Override
        public Object evaluateOperator(Object oLft, char opr, Object oRgt) {
            switch (opr) {
                case '=':
                    return oRgt;                                                                  // simple assignment, used as the final operation, must be maximum precedence
                case '^':
                    return Math.pow(toDouble(oLft), toDouble(oRgt));                                                    // power
                case '±':
                    return -toDouble(oRgt);                                                                 // unary negation
                case '*':
                    return toDouble(oLft) * toDouble(oRgt);                                                              // multiply (classical)
                case '×':
                    return toDouble(oLft) * toDouble(oRgt);                                                              // multiply (because it's a Unicode world out there)
                case '·':
                    return toDouble(oLft) * toDouble(oRgt);                                                              // multiply (because it's a Unicode world out there)
                case '(':
                    return toDouble(oLft) * toDouble(oRgt);                                                              // multiply (implicit due to brackets, e.g "(a)(b)")
                case '/':
                    return toDouble(oLft) / toDouble(oRgt);                                                              // divide (classical computing)
                case '÷':
                    return toDouble(oLft) / toDouble(oRgt);                                                              // divide (because it's a Unicode world out there)
                case '%':
                    return toDouble(oLft) % toDouble(oRgt);                                                              // remainder
                case '+':
                    return performAddition(oLft, oRgt);
                case '-':
                    return performSubstraction(oLft, oRgt);
                default:
            ...
            switch (Character.toLowerCase(fncnam.charAt(0))) {
                case 'a': {
                    if (fncnam.equalsIgnoreCase("abs")) {
                        return Math.abs(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("acos")) {
                        return Math.acos(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("asin")) {
                        return Math.asin(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("atan")) {
                        return Math.atan(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'c': {
                    if (fncnam.equalsIgnoreCase("cbrt")) {
                        return Math.cbrt(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("ceil")) {
                        return Math.ceil(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("cos")) {
                        return Math.cos(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("cosh")) {
                        return Math.cosh(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'e': {
                    if (fncnam.equalsIgnoreCase("exp")) {
                        return Math.exp(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("expm1")) {
                        return Math.expm1(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'f': {
                    if (fncnam.equalsIgnoreCase("floor")) {
                        return Math.floor(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'g': {
//              if(fncnam.equalsIgnoreCase("getExponent"   )) { return Math.getExponent(fncargs.next());                } needs Java 6
                ...
                case 'l': {
                    if (fncnam.equalsIgnoreCase("log")) {
                        return Math.log(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("log10")) {
                        return Math.log10(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("log1p")) {
                        return Math.log1p(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'm': {
                    if (fncnam.equalsIgnoreCase("max")) {
                        return Math.max(((Number) fncargs.next()).doubleValue(), ((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("min")) {
                        return Math.min(((Number) fncargs.next()).doubleValue(), ((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'n': {
//              if(fncnam.equalsIgnoreCase("nextUp"        )) { return Math.nextUp     (fncargs.next());                } needs Java 6
                ...
                case 'r': {
                    ...
                    if (fncnam.equalsIgnoreCase("round")) {
                        return Math.round(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("roundHE")) {
                        return Math.rint(((Number) fncargs.next()).doubleValue());
                    } // round half-even
                }
                break;
                case 's': {
                    if (fncnam.equalsIgnoreCase("signum")) {
                        return Math.signum(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("sin")) {
                        return Math.sin(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("sinh")) {
                        return Math.sinh(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("sqrt")) {
                        return Math.sqrt(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 't': {
                    if (fncnam.equalsIgnoreCase("tan")) {
                        return Math.tan(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("tanh")) {
                        return Math.tanh(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("toDegrees")) {
                        return Math.toDegrees(((Number) fncargs.next()).doubleValue());
                    }
                    if (fncnam.equalsIgnoreCase("toRadians")) {
                        return Math.toRadians(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                case 'u': {
                    if (fncnam.equalsIgnoreCase("ulp")) {
                        return Math.ulp(((Number) fncargs.next()).doubleValue());
                    }
                }
                break;
                // no default
...
// *************************************************************************************************
    ...
    public static Object performSubstraction(Object oLft, Object oRgt) {
        if (performCollectionOpp(oLft, oRgt)) {
            ((Collection) oLft).removeAll((Collection) oRgt);
            return oLft;

        } else if (performStringOpp(oLft, oRgt)) {
            return toString(oLft).replace(toString(oRgt), "");
        } else {
            return toDouble(oLft) - toDouble(oRgt);                                                              // add/unary-positive
        }                                                              // subtract/unary-negative
    }

    public static Object performAddition(Object oLft, Object oRgt) {
        if (performCollectionOpp(oLft, oRgt)) {
            ((Collection) oLft).addAll((Collection) oRgt);
            return oLft;

        } else if (performStringOpp(oLft, oRgt)) {
            return toString(oLft) + toString(oRgt);
        } else {
            return toDouble(oLft) + toDouble(oRgt);                                                              // add/unary-positive
        }
    }
    ...
    private static double toDouble(Object number) {
        if (number == null || number == NULL_VALUE) {
            return (double) NULL_VALUE.getValue();
        }
    ...
    private static boolean isNumber(Object number) {
        return number instanceof Number;

    }

    private static boolean performCollectionOpp(Object arg1, Object arg2) {
        boolean a1is = arg1 instanceof Collection;
        boolean a2is = arg2 instanceof Collection;
        if (a1is || a2is) {
            return true;
        }
    ...
    private static boolean performStringOpp(Object arg1, Object arg2) {

        boolean a1isNumber = isNumber(arg1);
        boolean a2isNumber = isNumber(arg2);
        if (a1isNumber && a2isNumber) {
            return false;
        }
        if (a1isNumber && arg2 == NULL_VALUE) {
            return false;
        }
        if (a2isNumber && arg1 == NULL_VALUE) {
            return false;
        }
        return true;
    }
...
// *************************************************************************************************
    public static void main(String[] args) {
        ...
        math.setFunctionHandler("sum", new SumFunction());
        math.setFunctionHandler("contains", new ContainsFunction());
        int i = 0;
        i++;
        // System.out.println("Middle: " + math.evaluate("floor((Right+1-Left)/2)+sum([1,2,3,4])+sum(Right+sum(Left)+Left)"));
        System.out.println("Middle: " + math.evaluate("sum([1,2,3,4])"));
        System.out.println("Middle: " + math.evaluate("[1,2,3,4]-[1,6,7]"));
        System.out.println("Middle: " + math.evaluate("contains([1,2,3,4]-[1,6,7],3)"));
        //System.out.println("Middle: " + math.evaluate("3==3"));
    }
} // END CLASS
